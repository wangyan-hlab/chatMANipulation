...
class FRCobot(object):
    ...
    def MoveJ(self, target_pos, target_flag="joint", 
              vel=50.0, ovl=100.0, exaxis_pos=np.zeros(4), blendT=-1.0, 
              offset_flag=0, offset_pos=np.zeros(6)):
        """
        功能: 控制机器人关节空间运动到目标位置
        ...
        tool = self.robot.GetActualTCPNum(0) #TODO:需要确定这里是否使用阻塞
        user = self.robot.GetActualWObjNum(0) #TODO:需要确定这里是否使用阻塞
        ...
        if target_flag == "joint":
            ...
            if getfk_ret[0] == 0:
                ...
                print("正运动学求解成功,关节位置为:", target_desc_pos)
            else:
            ...
            joint_pos_ref = self.robot.GetActualJointPosDegree(0) #TODO:需要确定这里是否使用阻塞
            ik_has_solution = self.robot.GetInverseKinHasSolution(target_desc_pos, joint_pos_ref)
            if ik_has_solution[0] == 0:
                if ik_has_solution[1]:
                    getikref_ret = self.robot.GetInverseKinRef(target_desc_pos, joint_pos_ref)
                    if getikref_ret[0] == 0:
                        target_joint_pos = getikref_ret[1:]
                        print("逆运动学求解成功,关节位置为:", target_joint_pos)
                    else:
    ...
    def MoveL(self, target_pos, target_flag="joint", 
              vel=50.0, ovl=100.0, exaxis_pos=np.zeros(4), blendR=-1.0, 
              search=0, offset_flag=0, offset_pos=np.zeros(6)):
        """
        功能: 控制机器人关节空间运动到目标位置
